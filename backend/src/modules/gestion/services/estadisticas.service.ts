...
export class EstadisticasService {
    ...
    async findAll () {
        ...
        const totalProyectosActivos = proyectos.filter(v => v.estado === EstadosProyectosEnum.ACTIVO);
        const totalProyectosfinalizados = proyectos.filter(v => v.estado === EstadosProyectosEnum.FINALIZADO);
        const totalProyectosBajas = proyectos.filter(v => v.estado === EstadosProyectosEnum.BAJA);
        proyecto["numeros"] = {
        ...
        const totalClientesActivos = clientes.filter(v => v.estado === EstadosClientesEnum.ACTIVO);
        const totalClientesBaja = clientes.filter(v => v.estado === EstadosClientesEnum.BAJA);
        cliente["numeros"] = {
        ...
        const totalTareasPendientes = tareas.filter(v => v.estado === Estados_Tareas.pendiente);
        const totalTareasFinalizadas = tareas.filter(v => v.estado === Estados_Tareas.finalizada);
        const totalTareasBajas = tareas.filter(v => v.estado === Estados_Tareas.baja);
        tarea["numeros"] = {
            total: tareas.length,
